...
export default function EditSubCategory() {
  ...
  // edit task data
useEffect(()=>{
  axios.get(`http://localhost:8000/subCategory/${id}`).then((response)=>{
  
  category.current.value=response.data.category; 
  subCategoryName.current.value= response.data.subCategoryName;
  subCategoryPhoto.current.value = response.data.subCategoryPhoto;
  ...
  const AddSubCategoryHandeler = ((e)=>{
      e.preventDefault();
          ...
          axios.put(`http://localhost:8000/subCategory/${id}`,update).then(()=>{
              
              Swal.fire({
              title: "Thanks",
              text: "Thanks your Sub category Update successfully",
              icon: "success"
              });

              Navigate('/admin-login/subcategory/manage');
          });
      e.target.reset();
  })
            ...
            <div className="flex-1">
                ...
                <div>
                    ...
                    <form className="max-w-2xl mx-auto" onSubmit={AddSubCategoryHandeler}>
                        ...
                        <button type="submit" className="text-white bg-gradient-to-r mt-5 from-green-400 via-green-500 to-green-600 hover:bg-gradient-to-br focus:ring-4 focus:outline-none focus:ring-green-300 dark:focus:ring-green-800 font-bold rounded-lg text-base px-5 py-2.5 text-center me-2 mb-2"><i className="text-xl bi bi-cloud-plus me-3"></i> UPDATE SUB CATEGORY</button>
